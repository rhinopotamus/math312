...
\begin{frame}{Goals for today:}
  \begin{enumerate}
    \item Define what \Alert{normal subgroups} are
...
\begin{frame}{Left vs.\ right cosets} %\Pause
  ...
  Because of our convention that arrows in a Cayley graph represent \Balert{right multiplication}:
  \begin{itemize}
...
\begin{frame}{Properties of cosets}
  \begin{block}{Proposition}
    ...
    every element $g\in G$ lives in \Balert{exactly one} (left) coset of $H$. ((Left) cosets never overlap.)
  \end{block}
  \begin{block}{Proposition}
  ...
  \end{block}
  \begin{block}{Definition}
    The \Alert{index} of a subgroup $H$ in $G$, written $[G:H]$, is the number of cosets of $H$ in $G$.
  \end{block}
  \begin{block}{Lagrange's theorem}
    If $H$ is a subgroup of a \Alert{finite} group $G$, then $|G| = [G:H]\cdot |H|$.
  \end{block}
\end{frame}
...
\begin{frame}{A different subgroup of $D_4$, $N = \<r\>$}
  ...
  \begin{exampleblock}{Informal definition}
    A subgroup for which every left coset is also a right coset is called \Alert{normal}.
  \end{exampleblock}
  \begin{alertblock}{Caveat!}
    Equality of cosets $xK=Kx$ \Balert{as sets} is \emph{different from} 
...
\begin{frame}{Normalizers}
  ...
  \begin{block}{Definition}
    ...
    \]
    i.e., the \Balert{union of reps of left cosets that are also reps of right cosets}. 
  \end{block} \Pause
