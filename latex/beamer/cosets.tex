...
\begin{frame}{Definition time! (soon)} 
  ...
  \begin{exampleblock}{Vibes-based explanation (not a definition)}
    \onslide<2->{
    Let $H\leq G$. A coset of $H$ is a \Balert{shifted copy} of $H$ somewhere else in $G$.
    }
  \end{exampleblock}
\onslide<3->{

  Example: consider $H = \<f\>\leq D_4$.
...
\onslide<4->{
  ...
  How do you shift to get from original $H$ to each of these shifted copies?

  \medskip
}
\onslide<11->{
  \begin{exampleblock}{\Alert{Left} cosets, and how to find them}
\onslide<12->{
    To find the \Alert{left} coset $\Alert{x}\Balert{H}$ in a Cayley graph,
    carry out the the following steps:
}
    \begin{enumerate}
\onslide<13->{
      \item starting from the identity, follow a path to get to \Alert{$x$} (``shift by $x$'')
}
\onslide<14->{
  ...
  \begin{exampleblock}{Left vs. right cosets}
    \begin{itemize}
      \item The \textbf{left coset} $\Alert{r}\Balert{H}$ in $D_4$: first
        \Alert{shift by $r$}, then traverse all
        ``\Balert{$H$-paths}''. \smallskip\Pause
      \item The \textbf{right coset} $\Balert{H}\Alert{r}$ in $D_4$:
        first traverse all \Balert{$H$-paths}, then \Alert{shift by $r$}.
      \end{itemize}
  \end{exampleblock}
...
\begin{frame}{Your turn!}
  ...
  \begin{exampleblock}{Reminder: finding left vs right cosets}
    \begin{itemize}
      \item Left coset $\Alert{x}\Balert{N}$: first \Alert{shift by $x$}, then traverse all \Balert{$N$-paths}
      \item Right coset $\Balert{N}\Alert{x}$: first traverse all \Balert{$N$-paths}, then \Alert{shift by $x$}
    \end{itemize}
...
\begin{frame}{Equality of sets vs.\ equality of elements}
  ...
  $rK$ and $Kr$ are the same \Alert{as sets}, even though the elements occur in a different order.
\end{frame}
...
\begin{frame}{Basic properties of cosets}
  ...
  \begin{block}{Proposition}
    ...
    every element $g\in G$ lives in \Balert{exactly one} coset of $H$.
  \end{block}
...
\begin{frame}{Funny historical aside}
  Guess who proved Lagrange's theorem. \Pause \Alert{Not Lagrange!}
  ...
  \begin{itemize}
    ...
    \begin{itemize}
      ...
      \item Take $H\leq S_n$ to be the set of permutations that \Alert{fix} the polynomial. $n! = |S_n| = [S_n : H] \cdot |H|$. \Pause
      \item The number of different polynomials is the number of cosets of $H$, aka the index $[S_n:H]$.\Pause
      \item So: true for \Alert{special subgroups} of $S_n$.\Pause
    \end{itemize}
    \item Gauss, 1801: the \Alert{special case} of subgroups of $(\Z/p\Z)^*$\Pause
    \item Cauchy, 1844: true for \Alert{any} subgroup $H \leq S_n$\Pause
    \item Jordan, 1861: true for any subgroup $H$ of a \Alert{permutation group}\Pause
    \begin{itemize}
      \item (so, now it's true for subgroups of \Alert{subgroups of $S_n$})\Pause
    \end{itemize}
