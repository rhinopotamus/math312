...
\begin{frame}{Normal subgroups in $D_4$}
  \begin{columns}
    ...
    \end{column}
    \begin{column}{0.5\textwidth}
      From our explorations, we found:\Pause
      \begin{itemize}
        \item $\<r\> \normal D_4$ \Pause (because it has index 2!) \Pause
        \item $\<r^2, f\> \normal D_4$ \Pause (index 2!) \Pause
        \item $\<r^2, rf\> \normal D_4$ \Pause (index 2!)
        \item $\<r^2\> \normal D_4$ \Pause (because it is $Z(D_4)$!)
        \begin{itemize}
          \item (Also, it's the only guy who's a subgroup of 3 different groups)
        \end{itemize}
      \end{itemize} \Pause
    \end{column}
  \end{columns}
...
\begin{frame}{How to check if a subgroup is normal} %\Pause
  ...
  \begin{alertblock}{Useful remark}
    ...
    \item $gHg^{-1}=H$ for all $g\in G$; \hfill (``\emph{only one \Balert{conjugate
...
\begin{frame}{Two pretty good reasons why $N$ is normal} \Pause
  ...
  \begin{enumerate}
    \item $N$ is the \Palert{only subgroup of its size} in the subgroup lattice of $A_4$, \\
...
\begin{frame}{Unicorn subgroups}
  ...
  \begin{exampleblock}{Very useful idea}
...
\begin{frame}{Unicorns in the diquaternion group} %\Pause
  ...
  Here's the lattice for a group called $\DQ_8$, which has order 16.
  ...
  (Preview: What can we say about the \Balert{conjugacy classes} of the subgroups of
...
\begin{frame}{A mystery group of order $16$} %\Pause
  ...
  Here is the subgroup lattice for some actual group of order $16$ but I'm not telling you which one it is. Find as many \Palert{unicorns} as you can.
  %%
  %% Subgroup lattice of a mystery group of order 16
  \[
  \begin{tikzpicture}[shorten >= -2pt, shorten <= -2pt,scale=.75]
    \begin{scope}[shift={(0,0)}]
...
\begin{frame}{``Reducing'' subgroup lattices}
  ...
  Left-subscripts denote the size of the conjugacy class. We call this a \Alert{subgroup diagram}. \\ (In some circumstances it might not actually be a lattice.)

  %% Subgroup DIAGRAM of A_5
  \[
  %%
  \hspace*{7mm}
  \begin{tikzpicture}[shorten >= -2pt, shorten <= -2pt,scale=.4,auto,xscale=2]
    %%
    \newcommand\Hh{16} % height of A_5
    \newcommand\Hg{9.5} % height of A_4
    \newcommand\Hf{8} % height of D_10
