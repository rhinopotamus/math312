...
\title[Quotient groups!]{Quotient groups!}
\subtitle{(and some review of cosets and normal subgroups)}
...
\begin{frame}{Goals for today:}
  \begin{enumerate}
    \item Define what \Alert{quotient groups} are
    \item See some examples 
    \item Thus, see why we care so much about normal subgroups
  \end{enumerate}
\end{frame}
...
\begin{frame}{Cosets!}
  ...
  \begin{exampleblock}{Morally:}
    A coset of $H$ is a shifted copy of $H$ somewhere else in $G$.
  \end{exampleblock} \pause

  \begin{alertblock}{A coset of $H$ is always / sometimes / never:}
    \begin{itemize}
      \item An element of $G$ \pause
      \item A subset of $G$ \pause
      \item Equal to $H$ \pause
...
\begin{frame}{Conjugate subgroups!} %\Pause
  ...
  \begin{block}{Definition}
    For a fixed element $g\in G$, the \alert{conjugate} of $H$ by $g$ is the set
      \[
      gHg^{-1}=\big\{ghg^{-1}\mid h\in H\big\}.
      \]
  \end{block} \pause
  
  \begin{alertblock}{A conjugate of $H$ is always / sometimes / never:}
    \begin{itemize}
      \item An element of $G$ \pause
      \item A subset of $G$ \pause
      \item Equal to $H$ \pause
  ...
  \begin{block}{Definition}
    The \alert{conjugacy class} of $H$ in $G$ is the set of all conjugates of $H$:
    \[\cl_G(H) = \{ gHg\inv \mid g\in G\}. \]
  \end{block}
  \begin{exampleblock}{Morally}
    $\cl_G(H)$ is a list of all the subgroups of $G$ that are ``similar to'' $H$.
  \end{exampleblock}
\end{frame}
...
\begin{frame}{Normal subgroups!}
  ...
  \begin{block}{Equivalent definition}
    \ldots if $gHg\inv = H$ for \emph{all} $g \in G$. 
  \end{block} \pause
  \begin{block}{Equivalent definition \#2}
    \ldots if there is only one conjugate subgroup to $H$, ie., $H$ itself. 
  \end{block} \pause
  \begin{block}{Equivalent definition \#3}
    \ldots if $|\cl_G(H)| = 1$.
  \end{block} \pause
  \begin{exampleblock}{Morally}
    Normal subgroups are in some way \Alert{unique} in their group.
  \end{exampleblock}
\end{frame}

%%====================================================================

\begin{frame}{Normal-\Alert{ish} subgroups}

  Okay, well, if $H \leq G$ \Alert{isn't} normal, then a natural followup question is:
  \[
  \text{``\emph{How} non-normal?''} \quad \pause
...
\begin{frame}{Normalizers!}
  \begin{block}{Definition}
    ...
    $N_G(H)$, is the set of elements $g\in G$ that ``normalize'' $H$:
    \begin{align*}
      N_G(H)&=\big\{g\in G\mid gH=Hg\big\} \\ \pause
      &=\big\{g\in G\mid gHg\inv=H\big\}
    \end{align*}
  \end{block} \pause
  \begin{alertblock}{The normalizer of $H$ always / sometimes / never:}
    \begin{itemize}
      \item An element of $G$ \pause
      \item A subset of $G$ \pause
      \item A subgroup of $G$ \pause
      \item Equal to $H$ \pause
      \item Contains $H$
    \end{itemize}
  \end{alertblock}
...
\begin{frame}{Three subgroups of $A_4$ (from Problem 9)}
...
\begin{frame}{Three subgroups of $A_4$ (from Problem 9)}
  Take $\Alert{a=(123)}$, $\Palert{b=(134)}$, $\Balert{x=(12)(34)}$, and $\Galert{z=(13)(24)}$. Then:
...
\begin{frame}[t]{Quotients} 
  ...
  We have already kinda bumped into the concept a quotient of a group by a subgroup:
  ...
  We now know enough algebra to be able to formalize this, but first some examples based on vibes. \medskip\Pause
...
\begin{frame}[t]{Quotients}
  ...
  Notice how taking a quotient generally \Balert{loses
    ...
    (You are squashing cosets together: $iN$ and $-iN$ are the same node.)
...
\begin{frame}[t]{Quotients}
  ...
  %%
  %% Failure of the quotient process for H=<(123> in A_4
  \[
  \begin{tikzpicture}[scale=.50,auto,bend angle=55]
    ...
    \begin{scope}[shift={(0,0)},scale=1]
      ...
      \draw [r] (m2) to (m3);
      \draw [r] (m3) to (m1);
      \node at (0,-3.75) {Cluster the left cosets};
      \node at (0,-4.4) {of $H=\<(123)\>$.};
    \end{scope}
    %%
    \begin{scope}[shift={(6,-1)},scale=1.3]
      %%
  ...
  We can still write $G/H:=\big\{H,\;xH,\;yH,\;zH\big\}$ for the set
  of (left) cosets of $H$ in $G$. \bigskip\Pause

  But now what in the hell are the \Balert{arrows}? \bigskip\pause

  Apparently all of those arrows are \Balert{$x$} arrows, but that doesn't make sense; \pause this is no longer a legit Cayley graph!
...
\begin{frame}[t]{When and why the quotient process works}
  ...
  In the following: \Balert{\emph{the right cosets $Hg$ are the ``arrowtips''}}.
  %%
  ...
  \Pause
  \begin{alertblock}{Key idea}
    For this process to work, the left cosets (nodes) and right cosets (arrows) must be \Balert{compatible}. \pause So if $H$ is a \Balert{normal subgroup} of $G$, then this process will work.
  \end{alertblock} \pause
  ...
  In other words, it \Balert{depends on where we start within $H$}.
...
\begin{frame}[t]{What does it mean to ``multiply'' two cosets?} 
  ...
  \begin{block}{Quotient theorem}\label{thm3:quotient}
    Consider the set of (left) cosets $G/H = \{eH, aH, bH, \ldots \}$.\pause

    If $H\normaleq G$, then $G/H$ forms a group, with
    binary operation 
    \[
    aH\cdot bH:= abH. 
    \]
  ...
  \begin{exampleblock}{Proof} \Pause
    ...
    \begin{center}\def\arraystretch{1.3}
      \begin{tabular}{rcllll}
        \multicolumn{3}{c}{Claim} &&& Data / Warrant \\\hline \pause
        $a_1H\cdot b_1H$ & $=$ & $a_1b_1H$ &&& \pause
        (by definition) \pause \\ \vspace{1mm}
        & $=$ & $a_1(b_2H)$ &&& \pause ($b_1H=b_2H$ by assumption) \pause \\
        & $=$ & $(a_1H)b_2$ &&& \pause ($b_2H=Hb_2$ since $H\normaleq G$) \pause \\
        & $=$ & $(a_2H)b_2$ &&& \pause ($a_1H=a_2H$ by assumption) \pause \\
        & $=$ & $a_2b_2H$ &&& \pause ($b_2H=Hb_2$ since $H\normaleq G$) \\ \pause
        & $=$ & $a_2H\cdot b_2H$ &&& \pause (\mbox{by definition}) 
      \end{tabular}
    \end{center}
...
\begin{frame}{(One last word on quotients)}
